...
export default {
	...
	options: [{
		...
		async executeAttribute(cmdData, levelOptions) {
			...
			return count_uses_per_user(guild, cmdData.guild);
		},
		 ...
		 * @param {ReceivedCommand} cmdData 
		 */
		execute(cmdData) { return displayCountPerUser(cmdData.guild); }
	}],
}
...
export async function countInvitesPerUserSorted(invites) {
	...
	per_user
		.filter((v, k) => k != null)
		.forEach((uses, user) => invitesSorted.push({ user, uses }));
...
export async function embedInvitesList(invites, currentGuild) {
	 ...
	 */
	const membersInGuild = await currentGuild?.members.fetch(invites.map(({ user }) => user))
		.catch(console.error);
