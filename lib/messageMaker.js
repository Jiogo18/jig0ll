...
function getColor(color) {
	...
	return colorLib.getColorByName(color)?.discordColor || colorLib.jig0ll_color;
}
...
export class EmbedMaker extends DiscordMessageEmbed {
	...
	getForMessage(cosmetic, other) {
		return new MessageOptions([this.getContent(cosmetic)], other?.components);
	}
}
