...
async function getMessageData(channel, id) {
	...
	if (messagesMatch.size > 1) {
		console.warn(`MessageData : plusieurs messages existent pour l'id ${id} (${messagesMatch.size} trouvés)`.yellow);
		messagesMatch.array().forEach((m, i) => i > 0 && m.delete());
	}
	return messagesMatch.first();
}
		...
		this.list = new TemporaryList(
			{
				...
				get: async (ID, previousValue) => {
					if (previousValue?.message?.fetch && !previousValue.message.deleted && parseAndMatchMessage(await previousValue.message.fetch(true), ID)) {
