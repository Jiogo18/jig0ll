...
class CommandContext {
	...
	static fromInteraction(interaction, bot) {
		return new CommandContext(bot.guilds.fetch(interaction.guild_id), bot.channels.fetch(interaction.channel_id), interaction.member.user);
	}
	static fromMessage(message) {
...
class CommandData {
	...
	get guild() { return this.bot.guilds.fetch(this.context.guild.id) || this.context.guild; }
	get channel() { return this.bot.channels.fetch(this.context.channel.id) || this.context.channel; }
	get author() { return this.bot.users.fetch(this.context.author.id) || this.context.author; }
	get source() {
	...
	async sendAnswer(message) {
		...
		try {
			...
			if(e.httpStatus == 404 && message.content != '') {
				const channel = this.bot.channels.fetch(this.context.channel_id);
