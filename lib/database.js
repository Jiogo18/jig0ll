...
async function getValue(name) {
	console.debug(`asking for the value ${name}`);
	console.debug(process.env.DATABASE_URL);
	const value = new Promise((resolve, reject) => {
		setTimeout(() => {resolve(undefined)}, 5000);
		keyv.get(name).then(v => { resolve(v) });
	})
	console.debug(`got it ! (${name}:${value}`);
	return value;
}
...
module.exports = class DataBase {
	...
	async getCurrentValue() {
		...
		if(this.#func && typeof this.#func.get == 'function') {
			return await this.#func.get(this.name) || this.data.value;
		}
