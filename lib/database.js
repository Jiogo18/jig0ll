import Keyv from 'keyv';//store PlenCity
var keyv = new Keyv(process.env.DATABASE_URL);//with SQLite (local) or Postgre (Heroku)
...
async function getValue(name) {
	return new Promise(async (resolve, reject) => {
		...
		resolve(await keyv.get(name));
	});
}
async function setValue(name, value) {
	return new Promise(async (resolve, reject) => {
		setTimeout(reject, 10000);
		resolve(await keyv.set(name, value));
	});
}
