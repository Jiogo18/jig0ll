...
export class PGDatabase extends PGClient {
	 ...
	 */
	async end() {
		return super.end().catch(e => console.error(`Error while disconnecting from the database`.red, e));
	}
 ...
 */
function getValueOfSupposedFunction(func) {
...
export class TemporaryValue {
	...
	async get() {
		...
		try {
			...
			console.error(`Error while getting a valuefor TemporaryValue`.red, err);
		}
