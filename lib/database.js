...
export class PGDatabase extends PGClient {
	 ...
	 */
	#waitConnect;
	#connected = false; get connected() { return this.#connected && this._connected; }
	/**
	...
	constructor(url, config) {
		...
		this.on('end', _ => this.#connected = false);//_connected est buggé donc on doit passer par une var externe...
		setTimeout(() => {//TODO: remove it
			plenitude = new KVDataBase(this, 'plenitude', {insertIfNotExist: true, createTable: true});
		}, 1000);	
	}
	/**
	...
	async connect() {
		...
		return this.#waitConnect = new Promise(async resolve => {
			...
			.catch(e => {
				console.error('Error while connecting to the database'.red, e)
	...
	async safeConnected() {
		...
		return this.connected || await this.connect();
	}
	 ...
	 */
	async end() { return new Promise(resolve => super.end().then(_=>resolve(true)).catch(_=>resolve(false))); }
...
if(!ok) {
	ok = true;
	setTimeout(async _ => {
		const mySql = new PGDatabase(process.env.DATABASE_URL);
		const plenitude = new KVDataBase(mySql, 'plenitude', {insertIfNotExist: true, createTable: true});
		// console.log('set PlenCitys : ', await plenitude.set('PlenCitys', 'Bayonne'));
		// console.log('PlenCitys is : ', await plenitude.get('PlenCitys'));
		// console.log('set PlenCity : ', await plenitude.set('PlenCity', 'Paris'));
		// console.log('PlenCity is : ', await plenitude.get('PlenCity'));
		//console.log('Delete : ', await plenitude.delete('PlenCitys'));
	}, 0);
}
...
async function getValue(name) {
	return plenitude.get(name);
}
async function setValue(name, value) {
	return plenitude.set(name, value);
}
