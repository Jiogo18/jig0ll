...
export class PGDatabase extends PGClient {
	 ...
	 */
	async end() { return super.end().catch(e => console.error(`Error while disconnecting from the database`.red, e)); }
 ...
 */
function getValueOfSupposedFunction(func) { return typeof func == 'function' ? func() : func; }
...
export class TemporaryValue {
	 ...
	 */
	canExpire() { return this.expireTime > -1; }
	/**
	...
	async get() {
		...
		try {
			this.value = await new Promise(async (res, err) => {
				...
				res(await getValueOfSupposedFunction(this.#obtainFunc?.get));
			});
			this.lastValue = this.value;//c'est ok, on l'a récupéré
		}
		catch(err) { console.error(`Error while getting a valuefor TemporaryValue`.red, e); }
	...
	async set(value) {
		...
		this.#reset = true;
		return new Promise(async res => { setTimeout(res, 5000); res(await this.#obtainFunc?.set?.(value)); });
	}
	#obtainFunc;
	/**
...
export class TemporaryList {
	 ...
	 */
	getRow(key) { return this.list.get(key) || this.createKey(key); }
	/**
	 * Get the value of the key
	 * @param {databaseKey} key The unique key of the value
	 * @returns The returns of the `get` function (of obtainFunc in the constructor)
	 */
	async get(key) { return this.getRow(key)?.get(); }
	/**
	 ...
	 */
	async set(key, value) { return this.getRow(key).set(value); }
	/**
	 * Reset every value in the list
	 */
	reset() { this.list.forEach(v => v.reset()); }
...
export class TemporaryKVData extends TemporaryList {
	...
	constructor(kvdatabase, timeout) {
		super({
			get: (n) => kvdatabase.get(n),
			set: (n, v) => kvdatabase.set(n, v)
		}, timeout);
...
export class TemporaryKVDatabase extends TemporaryKVData {
	 ...
	 * @param {PGDatabase} database The database
	 */
	setDatabase(database) { return this.kvdatabase.setDatabase(database); }
}